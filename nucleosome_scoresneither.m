...
function nucleosome_scoresCpGandnotCpG(chromNr1,chromNr2)


% load coordinates CpG
%filename = sprintf('../SkirmantasData/AllChromosomesSimulations/coordinatesCh1NMI.txt');

for chromNr=chromNr1:chromNr2
%filename = sprintf('../SkirmantasData/AllChromosomesSimulations/coordinatesCh%iCpGintersectNMI.txt',chromNr);
filename = sprintf('../SkirmantasData/AllChromosomesSimulations/coordinatesCh%neither.txt',chromNr);
...
name = sprintf('chr%i',chromNr);
index = find(strcmp(T.Var1 , name));
...
filename1 = sprintf('data/Yatzi_Harwood/Ch%iYazdineither.txt',chromNr);
writematrix(storeCpG,filename1)
